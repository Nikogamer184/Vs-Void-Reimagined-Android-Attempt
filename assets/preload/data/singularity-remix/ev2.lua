...
if not lowQuality then
	...
	setProperty('thethingie.alpha', 0)
		...
		if rus == true then
		setTextString('difficulty', 'не понял а где инди кросс')
		end
		if eng == true then
		setTextString('difficulty', 'wait where is indie cross')
		end
	...
	for i=7,4,-1 do
		...
		doTweenZoom('heisdyingholyshit', 'camGame', 1.1, 4, 'sineInOut')
	...
	if curStep == 2921 then
		...
		if rus == true then
		setTextString('difficulty', 'он спит не будите его')
		end
		if eng == true then
		setTextString('difficulty', "he's sleeping, don't wake him up")
		end
	end
